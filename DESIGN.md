...
- **Filters:** each row opens with an X chip (named "All topics" / "All sources" for assistive tech) that clears the filter, and the topic chips hold a single row. Topic and publisher chips reuse the navigation-active treatment (Instrument Blue when pressed) at `6px` radius, and the search field reuses the recessed input well. The masthead is the only card in the view, so hierarchy comes from typography and spacing rather than layered surfaces.
